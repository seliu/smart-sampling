...
def filter_data_batch( batch_data, filter_type = 'gaussian', window_size = 11, std = 2 ):
...
def filter_data( data, filter_type = 'gaussian', window_size = 11, std = 2 ):
    gaussian = scipy.signal.gaussian( M = window_size, std = std )
    gaussian /= sum( gaussian )
    convolved = numpy.convolve( data, gaussian, mode = 'same' )
    return convolved

def get_test_data( freq, n_offset = 10, random_seed = 42, shape_list = [ 'sawtooth', 'square', 'triangle' ], noise_ratio = 0 ):
...
def get_wave_data( freq, offset_list, duration = 0.1, framerate = 40000, shape_list = [ 'sawtooth', 'square', 'triangle' ] ):
    data_x, data_y = [], []
    ...
    for offset in offset_list:
        ...
        if 'sawtooth' in shape_list:
            data = get_sawtooth_wave( freq = freq, offset = offset, duration = duration, framerate = framerate )
            data_x.append( data )
            data_y.append( freq )

        # Square
        if 'square' in shape_list:
            data = get_square_wave( freq = freq, offset = offset, duration = duration, framerate = framerate )
            data_x.append( data )
            data_y.append( freq )
...
def get_sawtooth_wave( freq, offset, duration = 0.1, framerate = 40000 ):
